...
class Public_model extends CI_Model {
	...
	private function getFilter ( $big_get ) {
		...
		if ( $big_get['in_stock'] != '' ) {
			if ( $big_get['in_stock'] == 1 ) $sign = '>'; else
				$sign = '=';
	...
	public function getUserOrdersHistory ( $userId, $limit, $page ) {
		...
		$this->db->select( 'orders.*, orders_clients.first_name,' . ' orders_clients.last_name, orders_clients.email, orders_clients.phone, ' . 'orders_clients.address, orders_clients.city, orders_clients.post_code,' . ' orders_clients.notes, discount_codes.type as discount_type, discount_codes.amount as discount_amount' );
